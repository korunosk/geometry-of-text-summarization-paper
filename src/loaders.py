...
def load_embedded_topic(embedding_method, dataset_id, topic_id):
    fname = os.path.join(BASE_DATA_DIR, embedding_method, dataset_id, topic_id) + '_encoded.json'
    with open(fname, mode='r') as fp:
        return orjson.loads(fp.read())

def save_embedded_topic(embedding_method, dataset_id, topic_id, topic):
    directory = os.path.join(BASE_DATA_DIR, embedding_method, dataset_id)
...
def load_embedded_item(embedding_method, dataset_id, topic_id, item_id):
    fname = os.path.join(BASE_DATA_DIR, embedding_method, dataset_id, topic_id, item_id) + '.npy'
    return np.load(fname)

def save_embedded_item(embedding_method, dataset_id, topic_id, item_id, item):
    directory = os.path.join(BASE_DATA_DIR, embedding_method, dataset_id, topic_id)
    if not os.path.exists(directory):
        os.makedirs(directory)
