...
def KFoldTopics(topic_ids, n_splits):
    for i, (topic_ids_train_idx, topic_ids_test_idx) in enumerate(KFold(n_splits=n_splits).split(topic_ids)):
        yield i, (topic_ids[topic_ids_train_idx], topic_ids[topic_ids_test_idx])
...
def cross_validation_sampling(data, n_splits=3):
    ...
    for i, (topic_ids_train, topic_ids_test) in KFoldTopics(topic_ids, n_splits):
